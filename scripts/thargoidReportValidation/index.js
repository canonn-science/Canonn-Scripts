...
let scriptName = 'thargoidReportValidation';
let doReset = false;
let doValidate = true;
let isCron = true;
let jwt;
...
if (params.includes('--reset'.toLowerCase()) === true) {
	doReset = true;
	logger.warn('Forcefully resetting issue reports to pending');
}
		...
		for (l = 0; l < reportKeys.length; l++) {
			...
			if (reportCounts.data[reportKeys[l]].reports.pending > 0) {
				...
				for (v = 0; v < toValidate.length; v++) {
					...
					let reportChecked = await validateTools.thargoidReport(reportKeys[l], toValidate[v], jwt, bodyCache);
