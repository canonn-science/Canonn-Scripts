...
const { thargoidChecklist } = require('../checklist');
const { thargoidSchema } = require('../schemas');
const { thargoidValidate } = require('../validations');

let reportStatus = capi.reportStatus();

async function thargoidReport(reportType, reportData, jwt, bodyCache, url, settings) {
  let reportChecklist = await thargoidChecklist(reportType, reportData);
  logger.info('--> Running Validation Checks');

  // Do yup validation for missing data
  try {
    let checkMissing = await yupValidate(thargoidSchema.report, reportData);
  ...
  try {
    if (reportChecklist.stopValidation === false) {
      let checkSubtype = await checks.subtype(reportChecklist, url);

      if (checkSubtype) {
        reportChecklist = checkSubtype;
      } else {
        logger.warn('<-- Validation failed: Unknown Error on Subtype Check');
        reportChecklist.valid = reportStatus.network;
        reportChecklist.stopValidation = true;
      }
    }
  } catch (error) {
    logger.warn('<-- Script Error: Subtype Check');
    reportChecklist.valid = reportStatus.network;
    reportChecklist.stopValidation = true;
    console.log(error);
  }
  ...
  try {
    if (reportChecklist.stopValidation === false) {
      let checkCycle = await checks.cycle(reportChecklist, url);

      if (checkCycle) {
        reportChecklist = checkCycle;
      } else {
        logger.warn('<-- Validation failed: Unknown Error on Cycle Check');
        reportChecklist.valid = reportStatus.network;
        reportChecklist.stopValidation = true;
      }
    }
  } catch (error) {
    logger.warn('<-- Script Error: Cycle Check');
    reportChecklist.valid = reportStatus.network;
    reportChecklist.stopValidation = true;
    console.log(error);
  }
  ...
  let isValid = await thargoidValidate(reportChecklist);
