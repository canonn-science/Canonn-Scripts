...
const { guardianChecklist } = require('../checklist');
const { guardianSchema } = require('../schemas');
const { guardianValidate } = require('../validations');

let reportStatus = capi.reportStatus();

async function guardianReport(reportType, reportData, jwt, bodyCache, url, settings) {
  let reportChecklist = await guardianChecklist(reportType, reportData);
  logger.info('--> Running Validation Checks');

  // Do yup validation for missing data
  try {
    let checkMissing = await yupValidate(guardianSchema.report, reportData);
  ...
  let isValid = await guardianValidate(reportChecklist);
