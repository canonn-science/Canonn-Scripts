    ...
    if (status === 'new') {
      ...
      if (error === false) {
        if (reportchecklist.checks.capiv2.system.add === true) {
          ...
          let newsysData = await utils.processSystem(
            'edsm',
            reportchecklist.checks.edsm.system.data
          );
      ...
      if (error === false && data.system && data.body && data.discoveredBy) {
        ...
        if (!site.id) {
          ...
          logger.info(
            `<-- Created ${reportchecklist.report.site.toUpperCase()} Site ID: ${site.id}`
          );
      ...
      if (error === false) {
        if (Object.keys(data).length < 1) {
          ...
          let duplicateReport = await module.exports.invalid(
            'duplicate',
            reportchecklist,
            jwt,
            url
          ...
          logger.info(
            `<-- Updated ${reportchecklist.report.site.toUpperCase()} Site ID: ${site.id}`
          );
